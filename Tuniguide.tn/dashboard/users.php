...
<?php
  include_once('AdminDB.php');
		...
		if (isset($_POST['search'])) {
			$res=$db->getUsers($_POST['search']);
			if(mysqli_num_rows($res)==0) {
				echo '<script>alert ("User Not Found !!");</script>
				<script>location.href="users.php"</script>';
		...
		if ($row2['admin_post']=='admin') {
			$res3=$db->deleteClient($_POST['mailDel']);
			if ($res3==false) {
...
?>
  ...
  <form class="search-form" action="users.php" method="post">
  <input type="search" name="search" placeholder="Search In Users" class="search-input">
...
<table class="container">
	...
	<tbody>
		...
		<tr>
			<td><?php echo $row['nom_c']?></td>
			<td><?php echo $row['pren_c']?></td>
			<td><?php echo $row['mail_c']?></td>
			<td><?php echo $row['date_rec_c']?></td>
			<td><?php echo $row['country_c']?></td>
			<?php
			echo '
			<td><button style="background-color: red; color:white; cursor: pointer;" onclick=getMail("'.$row['mail_c'].'")>DELETE</button></td>';?>
		</tr>
...
<center>
<form method="post" action="users.php" name="f">
	<table>
		<tr>
