...
<html>
  ...
  <?php
		...
		$res=$db->getFeedback($_POST['search']);
		...
		if (isset($_POST['mailDel'])) {
			$res3=$db->deleteFeedback($_POST['mailDel']);
			if ($res3==false) {
  ...
  <!--*************************-->
  <center><h1>Feedback of clients : </h1></center>
  <form class="search-form" action="msg.php" method="post">
  <input type="search" name="search" placeholder="Search In Feedback" class="search-input">
...
<table class="container">
	...
	<?php while($row=mysqli_fetch_array($res)) { ?>
		<tr>
			...
			<td><?php echo ($row['rate_user']/5)*100; echo ' %'; ?></td>
			<td><?php echo $row['msg_user'] ?></td>
			<?php
			echo '
			<td><button style="background-color: red; color:white; cursor: pointer;" onclick=getUser("'.$row['mail_user'].'")>DELETE</button></td>';?>		</tr>		
		</tr>
		<?php }?>
	</tbody>
</table>
<center>
<form method="post" action="msg.php" name="f">
	<table>
		<tr>
			<td>
